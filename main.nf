...
workflow {
    ...
    if (!params.vcf_insertions) {
        error("Parameter --vcf_insertions is required!")
    }
    if (!params.vcf_deletions) {
        error("Parameter --vcf_deletions is required!")
    }
    if (!params.ref_fasta) {
        error("Parameter --ref_fasta is required!")
    }
    if (!params.consensus) {
        error("Parameter --consensus is required!")
    }
    if (!params.source_l1) {
        error("Parameter --source_l1 is required!")
    }
    if (!params.source_sva) {
        error("Parameter --source_sva is required!")
    }
    if (!params.motifs) {
        error("Parameter --motifs is required!")
    }
    if (!params.sva_vntr) {
...
workflow SVMODELLER {
    ...
    ch_ref_fasta_ready = ref_fasta.branch {
    ...
    ch_ref_fasta_decompressed = ch_ref_fasta_ready.uncompressed.mix(GUNZIP.out.gunzip)
